...
const Punjabi = () => {
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/tmdb/punjabi-movies`, {
                    params: { page },
                });
                if (page === 1) {
    ...
    >
        ...
        >
            <Typography 
                variant="h6" 
                sx={{ color: '#0fadbf', fontWeight: 'bold' }}
            >
                {selectedMovie?.title}
            </Typography>
        ...
        <iframe
            ...
            allowFullScreen
            style={{ flexGrow: 1 }} // Ensures iframe fills the available space
        ></iframe>
    </Box>
</Modal>
